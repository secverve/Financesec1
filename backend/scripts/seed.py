...
def seed():
    ...
    try:
        ...
        admin = User(email=settings.default_admin_email, full_name="Admin", password_hash=get_password_hash(settings.default_admin_password), role="ADMIN")
        user = User(email=settings.default_user_email, full_name="Trader Kim", password_hash=get_password_hash(settings.default_user_password), role="USER")
        db.add_all([admin, user])
        db.flush()

        user_account = Account(user_id=user.id, account_number="100-000-0001", cash_balance=50000000)
        admin_account = Account(user_id=admin.id, account_number="999-000-0001", cash_balance=0)
        db.add_all([user_account, admin_account])
        db.flush()

        db.add_all([
            UserBehaviorProfile(user_id=user.id, avg_order_amount=1200000, avg_daily_order_count=4, preferred_stocks="005930,000660", usual_login_regions="KR-SEOUL", usual_devices="device-main"),
            UserBehaviorProfile(user_id=admin.id, avg_order_amount=0, avg_daily_order_count=0, preferred_stocks="", usual_login_regions="KR-SEOUL", usual_devices="admin-console"),
        ])
        db.add_all([
            DeviceHistory(user_id=user.id, device_id="device-main", device_name="iPhone 17", last_ip_address="1.1.1.1", region="KR-SEOUL"),
            LoginHistory(user_id=user.id, email=user.email, ip_address="1.1.1.1", region="KR-SEOUL", success=False),
            LoginHistory(user_id=user.id, email=user.email, ip_address="1.1.1.1", region="KR-SEOUL", success=False),
            LoginHistory(user_id=user.id, email=user.email, ip_address="1.1.1.1", region="KR-SEOUL", success=False),
        ])
        ...
        suspicious_order = Order(user_id=user.id, account_id=user_account.id, stock_code="005930", side="BUY", order_type="LIMIT", quantity=120, price=83500, status="HELD", device_id="device-new", ip_address="203.0.113.10", region="OVERSEAS-SG", risk_score=110, risk_level="CRITICAL", decision="BLOCK")
        ...
        risk_event = RiskEvent(order_id=suspicious_order.id, user_id=user.id, stock_code="005930", stock_name="삼성전자", risk_score=110, risk_level="CRITICAL", decision="BLOCK", reason_summary="신규 디바이스 고액 주문; 해외 로그인 직후 주문; 로그인 실패 반복 후 주문")
        db.add(risk_event)
        db.flush()
        db.add_all([
        ...
        db.add(Execution(order_id=suspicious_order.id, stock_code="005930", executed_quantity=0, executed_price=0))
